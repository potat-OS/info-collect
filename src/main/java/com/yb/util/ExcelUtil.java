...
public class ExcelUtil {
    ...
    public void createSheet(String sheetName, String filePath, List<Student> students) {
        Workbook workbook = new XSSFWorkbook();
        Sheet sheet = workbook.getSheet(sheetName);
        Row title = sheet.createRow(0);
        int rowIndex = 0;
        for (int i = rowIndex; i < GetStuNums.EACH_STU_NUMS.length; i++) {
            title.createCell(i).setCellValue(rowName[i]);
        }
        rowIndex++;
